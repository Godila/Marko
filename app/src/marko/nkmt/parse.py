...
SPEC: list[ColumnSpec] = [
    ...
    ColumnSpec("Бренд", "brand", defaultable=True,
               hint="точное имя ТМ из НК; иначе правило РД → справочник бренда → дефолт"),
    ...
    ColumnSpec("Декларация", "declaration_number", defaultable=True,
               hint="номер из реестра; иначе правило РД → справочник бренда → дефолт"),
    ...
    ColumnSpec("", "producer", defaultable=True,
               hint="производитель: файл → правило РД → справочник бренда → дефолт; колонки в файле нет"),
