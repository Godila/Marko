...
значения apply_defaults не трогает, правило штампует 'rule'). Правило может
подставить декларацию (парой из реестра), производителя и дополнительные
поля карточки (parse.RULE_FIELDS). Условия правил сравниваются со значениями
строки ПОСЛЕ дефолтов («эффективное значение строки»); бренд и вид товара
матчатся без учёта регистра (casefold), вид — если входит в список правила.
Дата при подстановке берётся из записи реестра, на которую указывает правило,
— пара «номер-дата» всегда консистентна. Пишет только кэши справочников
(kv, brand_cache) — карточки/батчи не трогает.
"""
from marko.nkmt.dicts import get_defaults, get_rules
from marko.nkmt.parse import DEFAULTED_KEYS, PROV_KEYS, RULE_FIELDS, apply_defaults, parse_xlsx
...
def sources(raw: list[dict]) -> list[dict]:
    """Provenance значений строки: key (PROV_KEYS) → 'file' | 'default'."""
    return [{k: ("file" if str(r.get(k, "")).strip() else "default")
             for k in PROV_KEYS} for r in raw]


def match_rule(rules: list[dict], brand: str, product_type: str) -> dict | None:
    """Подошедшее правило: бренд и вид товара — casefold (ручной ввод мимо
    подсказок не должен тихо разваливать матчинг); вид должен входить в
    список правила; пустое условие = «любой». Из подошедших — больше непустых
    условий, ничья → больший id. Никого — None."""
    best, best_key = None, (-1, -1)
    low_type = product_type.casefold()
    for r in rules:
        score = 0
        if r["brand"]:
            ...
            score += 1
        if r["product_types"]:
            if low_type not in {t.casefold() for t in r["product_types"]}:
                continue
            score += 1
...
def apply_rules(rules: list[dict], rows: list[dict],
    ...
    Правило перебивает только дефолт-подстановку (слот 'default'): декларацию
    — парой номер + дата из записи правила, producer — если задан правилом,
    прочие поля карточки — по fields (ключи вне RULE_FIELDS игнорируются
    оборонительно: в БД значения могли попасть в обход роута).
    ...
    for row, s in zip(rows, src):
        ...
        if hit:
            ...
            for key in RULE_FIELDS:
                _fill(r, s2, key, str(fields.get(key, "")).strip(), "rule")
        out_rows.append(r)
        out_src.append(s2)
...
def resolve_fields(brand: str, product_type: str, defaults: dict,
    ...
    Для полей правила без подстановки src='' — источника нет."""
    ...
    for k in RULE_FIELDS:
        if k not in keys:
            keys[k] = {"value": rows[0].get(k, ""),
                       "src": src[0][k] if src[0][k] in ("rule", "file") else ""}
